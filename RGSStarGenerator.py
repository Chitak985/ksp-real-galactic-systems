...
class Star:
  def __init__(self, name, logGSurfaceGravity, luminositySun, tempK, massSuns, radiusSuns, rotationDays, description="Generated by a Python 3 script from a data array.", SIMBADdesg=None):
    self.name = name
    self.name2 = name.replace(" ","_").replace(".","").replace("-","_") # KSP-friendly name
...
while True:
  ...
  try:
    ...
    print("Automated retrieval failed. Please enter information manually or type 'skip' to skip the star and add it to the skip list.")
    q1 = input("[STAR "+str(counter)+"] Enter star name or skip: ").lower()
    if(q1 == "stop"):
      break
    elif(q1 == "skip"):
      q1 = input("[STAR "+str(counter)+"] Enter star name: ").lower()
      with open("RealGalacticSystems/Misc/SkippedSystems.txt","a") as f:
        f.write(q1+"\n")
      continue
...
for i in stars:
  orbit = AstroToKSP.makeKoprnicusOrbit(AstroToKSP.skycoord_to_ksp_orbit(AstroToKSP.getSimbadStar(i.SIMBADname,i.name)))
